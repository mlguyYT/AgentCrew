...
- local tool availability such as `git`, the project detector, the task classifier, task intake, task brief, work plan, readiness check, and the status dashboard
- setup docs such as installation, automatic loading, project detection, task classifier, task intake, acceptance criteria, work planning, implementation readiness, status dashboard, and human decision queue docs
